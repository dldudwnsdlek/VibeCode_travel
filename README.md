...
LocalHub는 서울 지역의 공공데이터를 한 화면에 모으고 다음 기능을 제공하여 이 문제를 해결합니다.
...
- [ ] 필수 기능을 100% 구현한다.
- [ ] 선택 기능을 최소 1개 이상 실제 배포한다.
- [ ] Netlify 배포 URL에서 주요 기능이 정상 동작한다.
- [ ] 모바일 화면에서도 게시판, 챗봇, 지도를 사용할 수 있다.
- [ ] 데이터 출처·라이선스·수집일을 기능 명세서에 기록한다.
- [ ] 산출물 5종을 마감 전에 모두 제출한다.
...
- [ ] 게시글을 작성할 수 있다.
- [ ] 새로고침 후에도 같은 브라우저에서 게시글이 유지된다.
- [ ] 목록에서 게시글 상세 화면으로 이동할 수 있다.
- [ ] 올바른 비밀번호로 게시글을 수정할 수 있다.
- [ ] 잘못된 비밀번호로는 수정할 수 없다.
- [ ] 올바른 비밀번호로 게시글을 삭제할 수 있다.
- [ ] 빈 제목·내용 등 잘못된 입력을 방지한다.

### 챗봇

- [ ] 플로팅 버튼으로 챗봇을 열고 닫을 수 있다.
- [ ] 관광지 질문에 제공 데이터 기반 답변을 한다.
- [ ] 맛집 위치 질문에 관련 장소를 안내한다.
- [ ] 축제 일정 질문에 데이터가 있는 범위에서 답변한다.
- [ ] 게시글 검색 질문에 관련 게시글을 찾는다.
- [ ] 대화 히스토리가 유지된다.
- [ ] API 오류 시 사용자에게 이해할 수 있는 메시지를 표시한다.
- [ ] 관련 데이터가 없을 때 임의로 정보를 만들지 않도록 안내한다.
- [ ] 응답 길이와 호출 횟수가 비용 한도를 넘지 않도록 제한한다.

### 지도

- [ ] Kakao 지도가 정상적으로 표시된다.
- [ ] 관광지와 맛집 마커가 올바른 위치에 나타난다.
- [ ] 좌표가 없는 데이터는 안전하게 제외된다.
- [ ] 카테고리 필터가 정상 동작한다.
- [ ] 마커 클릭 시 상세정보가 표시된다.
- [ ] 모바일 화면에서 지도를 조작할 수 있다.

### 배포·보안·산출물

- [ ] `.env` 파일이 Git에 포함되지 않았다.
- [ ] OpenAI API 키가 프론트엔드 코드에 직접 노출되지 않았다.
- [ ] API 사용량과 결제 한도가 설정되어 있다.
- [ ] Netlify 배포 URL에서 전체 기능이 동작한다.
- [ ] 기능 명세서에 데이터 출처·라이선스·수집일이 있다.
- [ ] 산출물 5종이 모두 준비되었다.
...
MVP 완료 이후에는 다음 기능을 단계적으로 검토할 수 있습니다.
...
**LocalHub는 짧은 개발 기간 안에 필수 기능을 안정적으로 완성하고, 지도 시각화로 사용자 체감 효과를 높이는 것을 최우선으로 합니다.**
